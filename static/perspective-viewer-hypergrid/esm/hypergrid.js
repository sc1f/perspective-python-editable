...
bindTemplate(TEMPLATE, style)(class HypergridElement extends _CustomElement {
  ...
  connectedCallback() {
    if (!this.grid) {
      ...
      grid_properties.renderer = ["SimpleCell", "Borders"]; // Handle grouped header plugin bugs
      ...
      this.grid.installPlugins([perspectivePlugin, [groupedHeaderPlugin, grid_properties.groupedHeader]]); // Broken in fin-hypergrid-grouped-header 0.1.2
      ...
      set_editors(this.grid); // Add tree cell renderer

      this.grid.cellRenderers.add("TreeCell", Base.extend({
        paint: treeLineRendererPaint
      }));
...
const plugin = {
  ...
  save: function () {
    ...
    if (hypergrid && hypergrid.selectionModel.hasRowSelections()) {
      return {
        selected: hypergrid.behavior.dataModel.getSelectedRowID()
      };
    }
  },
...
//# sourceMappingURL=hypergrid.js.map