...
function setPSP(payload, force = false) {
  ...
  this.grid.addProperties(this.grid.get_dynamic_styles(selectable)); // Following call to setData signals the grid to call createColumns and
  ...
  if (!force && this._memoized_schema && isEqual(this._memoized_schema.slice(0, this._memoized_schema.length), new_schema.slice(0, new_schema.length)) && isEqual(payload.rowPivots, this._memoized_pivots)) {
...
function sortColumn(event) {
  ...
  const already_sorted = item_index > -1; // shift key to enable abs sorting alt key to not remove already sorted
  // columns
  ...
  let new_sort_direction; // if the column is already sorted we increment the sort
...
async function getCellConfig(row_idx, col_idx) {
  ...
  const r = await this.dataModel._view.to_json({
    start_row,
    end_row
  });
...
export const install = function (grid) {
  ...
  grid.addEventListener("fin-row-selection-changed", async function (event) {
    ...
    const detail = selected ? await getCellConfig.call(event.detail.grid.behavior, rows[0]) : {
      config: {
        filters: []
      }
    };
    event.detail.grid.canvas.dispatchEvent(new CustomEvent("perspective-select", {
      bubbles: true,
      composed: true,
      detail: {
        selected,
  ...
  Object.getPrototypeOf(grid.behavior).getCursorAt = function (x, y, event) {
    var _this$dataModel$data, _this$dataModel$data2, _this$dataModel$data3;

    const rp_len = (_this$dataModel$data = this.dataModel.data[y - 1]) === null || _this$dataModel$data === void 0 ? void 0 : (_this$dataModel$data2 = _this$dataModel$data[-1]) === null || _this$dataModel$data2 === void 0 ? void 0 : (_this$dataModel$data3 = _this$dataModel$data2.rowPath) === null || _this$dataModel$data3 === void 0 ? void 0 : _this$dataModel$data3.length;
    const is_select = !!this.grid.properties.rowSelection;

    if (is_select) {
      if (x === -1 && rp_len <= this.dataModel._config.row_pivots.length && (event === null || event === void 0 ? void 0 : event.gridPoint.x) <= rp_len * 15 + 10) {
        return "pointer";
      }
    } else {
  ...
  Object.getPrototypeOf(grid.behavior).cellClicked = async function (event) {
    var _this$dataModel$data$, _this$dataModel$data$2, _this$dataModel$data$3;
    ...
    const filters = config.filter.concat(row_filters).concat(column_filters); // this only works for single row select (which is all we support
    // right now)
    ...
    if (this.grid.properties.rowSelection) {
      ...
      this.grid.canvas.dispatchEvent(new CustomEvent("perspective-select", {
        bubbles: true,
        composed: true,
        detail: {
          selected,
    ...
    this.grid.canvas.dispatchEvent(new CustomEvent("perspective-click", {
      bubbles: true,
      composed: true,
      detail: await getCellConfig.call(this, y, x)
    }));
    const rp_len = (_this$dataModel$data$ = this.dataModel.data[y]) === null || _this$dataModel$data$ === void 0 ? void 0 : (_this$dataModel$data$2 = _this$dataModel$data$[-1]) === null || _this$dataModel$data$2 === void 0 ? void 0 : (_this$dataModel$data$3 = _this$dataModel$data$2.rowPath) === null || _this$dataModel$data$3 === void 0 ? void 0 : _this$dataModel$data$3.length;
    const is_toggle = !this.grid.properties.rowSelection || (event === null || event === void 0 ? void 0 : event.gridPoint.x) <= rp_len * 15 + 10;
  ...
  grid.canvas.hasFocus = function () {
    ...
    if (!((_grid$div = grid.div) === null || _grid$div === void 0 ? void 0 : _grid$div.isConnected)) {
      return;
    }
  ...
  grid.lookupFeature("CellSelection").handleDOWN = function (grid, event) {
    ...
    if (!grid.cellEditor) {
      ...
      let {
        x,
        y
      } = grid.selectionModel.getLastSelection().origin;
  ...
  grid.canvas.resize = async function (force) {
    ...
    if (isHIDPI) {
      ...
      const backingStoreRatio = this.gc.webkitBackingStorePixelRatio || this.gc.mozBackingStorePixelRatio || this.gc.msBackingStorePixelRatio || this.gc.oBackingStorePixelRatio || this.gc.backingStorePixelRatio || 1;
...
//# sourceMappingURL=perspective-plugin.js.map