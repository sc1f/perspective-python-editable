...
function setEditorValueText(updated) {
  ...
  this.input.addEventListener("keydown", keydown.bind(this)); // refire mouseover so hover does not go away when mouse stays over edit
  // cell

  this.input.addEventListener("mouseover", event => {
    var mouseMoveEvent = document.createEvent("MouseEvents");
    mouseMoveEvent.initMouseEvent("mouseover", event.canBubble, event.cancelable, window, event.detail, event.screenX, event.screenY, event.clientX, event.clientY, event.ctrlKey, event.altKye, event.shiftKey, event.metaKey, event.button, event.relatedTarget);
...
function getEditorValueText(updated) {
  this._row.then(([old]) => {
    ...
    const port_id = this.grid.behavior.dataModel._viewer._edit_port_id || 0;
    console.log(port_id, this.grid.behavior.dataModel);
...
function getEditorValueNumber(updated) {
  this._row.then(([old]) => {
    ...
    this._table.update([{
      __INDEX__: index,
      [colname]: Number(updated.replace(/,/g, ""))
    }], {
      port_id: port_id
    });
  });
...
function stopEditing(feedback) {
  ...
  if (!error) {
    const {
      x,
      y
    } = this.grid.selectionModel.getLastSelection().origin;
