...
PerspectiveViewer = (_dec = bindTemplate(template, view_style, default_style), _dec(_class = (_class2 = class PerspectiveViewer extends ActionElement {
  constructor() {
    super();
  ...
  sort(sort) {
    ...
    this._update_column_list(sort, inner, (s, computed_names) => {
      let dir = "asc";
  ...
  "computed-columns"(computed_columns) {
    ...
    (async () => {
      ...
      this._reset_computed_column_view(to_remove); // Always store a copy of the parsed computed columns for
  ...
  filters(filters) {
    ...
    if (!this._updating_filter) {
      ...
      this._update_column_list(filters, inner, (filter, computed_names) => {
  ...
  clear() {
    ...
    (_this$_table = this._table) === null || _this$_table === void 0 ? void 0 : _this$_table.clear();
  }
  /**
