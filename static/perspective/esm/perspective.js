...
import papaparse from "papaparse"; // IE fix - chrono::steady_clock depends on performance.now() which does not
// exist in IE workers
...
export default function (Module) {
  ...
  table.prototype.update = function (data, options) {
    ...
    if (data instanceof ArrayBuffer) {
      ...
      accessor.init(papaparse.parse(data.trim(), {
        header: true
      }).data);
  ...
  const perspective = {
    ...
    table: function (data, options) {
      ...
      try {
        const op = __MODULE__.t_op.OP_INSERT; // Always create new tables using port 0
