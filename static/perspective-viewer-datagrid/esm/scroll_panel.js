...
export let DatagridVirtualTableViewModel = (_class = class DatagridVirtualTableViewModel extends _CustomElement {
  constructor() {
    super();
  }
  /**
  ...
  create_shadow_dom() {
    ...
    this.shadowRoot.innerHTML = html` <style> ${CONTAINER_STYLE + MATERIAL_STYLE} </style> <div class="pd-scroll-container"> <div class="pd-virtual-panel">${this._virtual_scrolling_disabled ? slot : ""}</div> <div class="pd-scroll-table-clip">${this._virtual_scrolling_disabled ? "" : slot}</div> <div style="position: absolute; visibility: hidden;"></div> </div> `;
  ...
  _calculate_viewport(nrows, reset_scroll_position) {
    ...
    const {
      start_row,
      end_row
    } = this._calculate_row_range(nrows, reset_scroll_position);

    const {
      start_col,
      end_col
    } = this._calculate_column_range();
  ...
  _swap_in(args) {
    this._render_element.dispatchEvent(new CustomEvent("perspective-datagrid-before-update", {
      bubbles: true,
      detail: this
    }));
  ...
  _swap_out(args) {
    ...
    this._render_element.dispatchEvent(new CustomEvent("perspective-datagrid-after-update", {
      bubbles: true,
      detail: this
    }));
  }
  ...
  infer_options(config) {
    ...
    delete config["sort"];
    return {
      reset_scroll_position: !isEqual(config, old_config)
    };
  }
  /**
...
}, (_applyDecoratedDescriptor(_class.prototype, "draw", [throttlePromise], Object.getOwnPropertyDescriptor(_class.prototype, "draw"), _class.prototype)), _class);
//# sourceMappingURL=scroll_panel.js.map