...
export let DatagridViewEventModel = (_class = class DatagridViewEventModel extends DatagridVirtualTableViewModel {
  register_listeners() {
    this._scroll_container.addEventListener("scroll", this._on_scroll.bind(this), {
      passive: false
    });

    this._scroll_container.addEventListener("mousewheel", this._on_mousewheel.bind(this), {
      passive: false
    });
  ...
  async _on_select(metadata) {
    ...
    this._render_element.dispatchEvent(new CustomEvent("perspective-select", {
      bubbles: true,
      composed: true,
      detail: {
...
}, (_applyDecoratedDescriptor(_class.prototype, "_on_resize_column_move", [throttlePromise], Object.getOwnPropertyDescriptor(_class.prototype, "_on_resize_column_move"), _class.prototype)), _class);
